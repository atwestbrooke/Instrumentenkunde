...
document.addEventListener("keydown", function(e) { if ((80 === e.keyCode) ||
                                                      ...
                                                      for (i = 0; 1 < audioList.length; i++) {
                                                          document.getElementById("audio-player-" + i).classList.remove("aktivplayer");
                                                          document.getElementById("audioplayer-" + i).style.opacity = "1.0";
                                                      }
                                                      }
                                                  }, false);
...
document.addEventListener("keydown", function(e) { if (13 == e.keyCode) { activeAudio.currentTime = 0.0001}}, false);
...
let spacePlay = function () {
	var index = parseFloat(activePlayButton.id.slice(-1));
	audioList[index].play(); 
};

 AudioObject.prototype.clickActivate = function () {
     let testval = parseFloat(event.target.id.slice(-1));
     activePlayButton = document.getElementById("playbutton-" + testval);
     activeAudio = document.getElementsByClassName("audio")[testval];
     if ((event.target.id == "audioplayer-" + testval) || (event.target.id == "playbutton-" + testval) || (event.target.id == "timeline-" + testval) || (event.target.id == "playhead-" + testval) || (event.target.id == "container" + testval) || (event.target.id == "AudioScore" + testval) || (event.target.id == "AudioScore" + testval) || (event.target.id == "vbButton" + testval)) {
         document.getElementById("audio-player-" + testval).classList.add("aktivplayer");
         document.getElementById("audioplayer-" + testval).style.opacity = "0.92";
         for (i = 0; 1 < audioList.length; i++) {
            if (testval != i) {
                 document.getElementById("audio-player-" + i).classList.remove("aktivplayer");
                 document.getElementById("audioplayer-" + i).style.opacity = "1.0";
